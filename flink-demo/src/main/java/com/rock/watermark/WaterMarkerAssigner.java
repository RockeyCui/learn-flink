...
public class WaterMarkerAssigner {
    ...
    public DataStream assignWaterMarker(DataStream<Row> dataStream, String tableName, RowTypeInfo typeInfo, String eventTimeFieldName, long maxOutOrderness) {
        ...
        if (fieldType.getTypeClass().getTypeName().equalsIgnoreCase("java.sql.Timestamp")) {
            waterMarker = new CustomerWaterMarkerForTimeStamp(Time.milliseconds(maxOutOrderness), pos, TimeZone.getDefault().getID());
        } else if (fieldType.getTypeClass().getTypeName().equalsIgnoreCase("java.lang.Long")) {
            waterMarker = new CustomerWaterMarkerForLong(Time.milliseconds(maxOutOrderness), pos, TimeZone.getDefault().getID());
        } else {
            throw new IllegalArgumentException("not support type of " + fieldType + ", current only support(timestamp, long).");
        }

        String fromTag = "Source:" + tableName;
